...
## **A.4 Multi-Parent Vector Composition Algorithm (Compose())**

During identity graph synthesis (Merge()), parent genesis vectors ($\\mathbf{v}\_{genA}, \\mathbf{v}\_{genB} \\in \\mathbb{S}^{d-1}$) MUST NOT be replaced with cryptographic hash bit-strings. Compliant backends SHALL derive composite floor vectors using Normalized Weighted Centroid Composition:

$$\\text{Compose}(\\mathbf{v}\_{genA}, \\mathbf{v}\_{genB}) \= \\mathbf{v}\_{genesis,child} \= \\frac{\\alpha \\mathbf{v}\_{genA} \+ \\beta \\mathbf{v}\_{genB}}{\\|\\alpha \\mathbf{v}\_{genA} \+ \\beta \\mathbf{v}\_{genB}\\|\_2 \+ \\epsilon}$$  
Where weights $\\alpha, \\beta \> 0$ default to $\\alpha \= 0.5, \\beta \= 0.5$ for equal merges, and $\\epsilon \= 10^{-12}$. The resulting vector $\\mathbf{v}\_{genesis,child}$ is guaranteed to lie on the unit hypersphere $\\mathbb{S}^{d-1}$, preserving geometric compatibility with dot-product cosine similarity checks.

## **A.5 Multi-Horizon Cosine Evaluation Formula**
...
| **Genesis Floor** | All Constituent Parent Genesis Anchors ($\\forall i \\in \\text{Parents}, V\_{genesis,i}$) | $\\forall i, \\quad \\mathbf{v}\_R \\cdot \\mathbf{v}\_{genesis,i} \\ge \\tau\_{genesis} \\quad (0.70)$ | ERR\_GENESIS\_FLOOR\_VIOLATION |

## **A.6 Heterogeneous Substrate Compatibility & Cross-Model Projections**
...
\---  
Appendix A Status: Version 1.2 Normative Baseline  
Track: Normative  
Bound to Specification Stack: Technical Specification v11.1  
