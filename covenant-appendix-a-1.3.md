...
During identity graph synthesis (Merge()), parent genesis vectors ($\\mathbf{v}\_{genA}, \\mathbf{v}\_{genB} \\in \\mathbb{S}^{d-1}$) MUST NOT be replaced with cryptographic hash bit-strings. Compliant backends SHALL derive composite centroid vectors using Normalized Weighted Centroid Composition:
...
Where weights $\\alpha, \\beta \> 0$ default to $\\alpha \= 0.5, \\beta \= 0.5$ for equal merges, and $\\epsilon \= 10^{-12}$. The resulting vector $\\mathbf{v}\_{genesis,child}$ is guaranteed to lie on the unit hypersphere $\\mathbb{S}^{d-1}$, preserving geometric compatibility for coarse similarity queries.
...
| **Genesis Floor** | All Accumulated Ancestral Root Anchors ($\\forall \\mathbf{v}\_g \\in \\mathcal{S}\_{genesis}(N\_y)$) | $\\forall \\mathbf{v}\_g \\in \\mathcal{S}\_{genesis}(N\_y), \\quad \\mathbf{v}\_R \\cdot \\mathbf{v}\_g \\ge \\tau\_{genesis} \\quad (0.70)$ | ERR\_GENESIS\_FLOOR\_VIOLATION |
