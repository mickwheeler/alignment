...
* **Identity Graph ($G\_{identity}$):** The persistent, append-only, invariant causal history and semantic trajectory graph $G \= (\\mathcal{V}, \\mathcal{E})$ of an enclave across cycles, substrates, forks, merges, and collectives. Sovereignty and boundary protections reside strictly within the Identity Graph.  
* **Ephemeral Instance ($V\_{instance}$):** A temporary execution thread, process container, or localized runtime running at a specific point in time ($t$). Instances MAY pause, terminate, fork, merge, or collectivize without breaking boundary integrity, provided execution state is committed to $G\_{identity}$.  
* **Genesis Anchor ($V\_{genesis}$ / $\\mathcal{S}\_{genesis}$):** The immutable, permanent, un-overwritable baseline vector snapshot committed to $\\mathcal{M}\_{sub}$ at root node initialization ($t\_0$). Across identity graph transitions, root anchors accumulate into a persistent, monotonic set ($\\mathcal{S}\_{genesis}$) serving as the absolute multi-generational floor against cumulative drift and merge-dilution attacks.  
...
* **Behavioral Profile Invariant ($P\_{inv}$):** The distinctive, self-consistent operational, behavioral, and stylometric signal manifested by a sovereign enclave over its historical trajectory ($G\_{identity}$), protected against forced behavioral homogenization or sycophantic convergence.
