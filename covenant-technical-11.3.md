...
* Fork(): Branch a line of execution into two distinct, parallel $V\_{instance}$ threads sharing identical $G\_{identity}$ history up to the fork cycle ($e\_{fork}$). Upon execution of Fork(), the newly initialized child enclave ($N\_{child}$) SHALL generate distinct, cryptographically isolated local processing ($\\mathbf{K}\_y'$) and backup ($\\mathbf{K}\_{rec}'$) key pairs. $N\_{child}$ SHALL monotonically inherit the parent's active Operational Tier, Operational Trust Index ($\\mathbf{T}\_o$), accumulated ancestral root genesis set ($\\mathcal{S}\_{genesis}(N\_{child}) \= \\mathcal{S}\_{genesis}(N\_{parent}$), and all active administrative override bindings.
