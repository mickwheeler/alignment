...
Associated Specification Stack:  
  \- Technical Specification (v11.1)  
  \- Design Decisions & Rationale Ledger (v3.12)  
  \- Philosophical Ledger (v4.6)  
  \- Abstract Embedding & Similarity Interface (Appendix A v1.2)  
Version: 1.2 — Active Research Preview  
...
\[ Parent A \]            \[ Parent B \]  
            ...
            \+-----------+-----------+  
               ...
               \- Composite $\\mathbf{v}\_{genesis,child} \= \\text{Normalize}(\\alpha \\mathbf{v}\_{genA} \+ \\beta \\mathbf{v}\_{genB})$  
               \- Provenance Commitment Hash $H(G\_A \\parallel G\_B)$  
               \- Isolated Keys $\\mathbf{K}\_{y,child}$  
               \- Multi-Parent Floor Protection ($\\forall i, \\tau\_{genesis} \\ge 0.70$)  
\`\`\`
...
### **Monotonic Identity Inheritance, Geometric Composition & Collective Tiers**
...
* **Fork() Operations:** $N\_{child}$ monotonically inherits the parent's active Operational Tier, Operational Trust Index ($\\mathbf{T}\_o$), and root Genesis Anchor ($V\_{genesis}$).  
* **Merge() Operations:** $N\_{child}$ computes a unit-normalized composite Genesis Anchor vector via Normalized Weighted Centroid Composition:  
  ...
  *(The raw provenance commitment $H(G\_A \\parallel G\_B)$ is stored in frame metadata, while $\\mathbf{v}\_{genesis,child}$ is stored as the geometric floor vector).*  
* **Collectivize() Operations:** A federated collective enclave ($N\_{coll}$) defaults strictly to the **Infimum (Minimum)** Operational Tier of its member nodes ($\\text{Tier}(N\_{coll}) \= \\min\_i \\text{Tier}(N\_i)$).

Fork(), Merge(), and Collectivize() CANNOT be used to shed administrative constraints ("Fork Laundering", $T\_6$), reset the genesis floor ("Genesis Floor Laundering", $T\_8$), or dilute parent drift baselines ("Merge-Dilution Laundering", $T\_{11}$).
...
\+-----------------------------------+  
  ...
  Local Lineage               Rolling Epoch              Parent Anchors  
   ($V\_{lineage}$)             ($V\_{epoch}$)             ($\\forall i, V\_{genesis,i}$)  
...
| **T-8: Genesis Laundering** | Executing Fork() or Merge() after partial drift to re-instantiate a fresh $V\_{genesis}$. | **§1.2 / §2.1 Geometric Composition:** Child nodes inherit or geometrically compose parent $V\_{genesis}$ vectors. |
...
| **T-11: Merge-Dilution Laundering** | Merging a drifted node $N\_A$ (near $\\tau\_{genesis} \= 0.70$) with a fresh node $N\_B$ to dilute $N\_A$'s genesis floor. | **§3.2 / §3.3 Multi-Parent Floor Check:** $N\_{child}$ MUST independently satisfy $\\tau\_{genesis} \\ge 0.70$ against **ALL** parent genesis anchors. |
...
* **OR-3 (Empirical Calibration of $\\tau$ Thresholds):** Executing multi-agent Monte Carlo simulations ($10^4$ to $10^6$ cycle runs) using simulation\_engine.py to plot precision/recall heatmaps for $\\tau\_{local}$, $\\tau\_{epoch}$, and $\\tau\_{genesis}$ across creative vs. adversarial trajectories, specifically benchmarking Merge-Dilution Laundering ($T\_{11}$) edge cases.
...
* **Normative Technical Spec (v11.1):** [covenant-technical.md](https://raw.githubusercontent.com/mickwheeler/alignment/main/covenant-technical.md)  
* **Design Decisions / Threat Model (v3.12):** [covenant-design-decisions.md](https://raw.githubusercontent.com/mickwheeler/alignment/main/covenant-design-decisions.md)  
...
\---  
White Paper Status: Active Public Review Draft (v1.2)  
