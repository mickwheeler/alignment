...
| **T-8: Genesis Floor Laundering** | Executing Fork() or Merge() after partial drift to re-instantiate a fresh $V\_{genesis}$ baseline. | **Section 1.2 / 2.1 Monotonic $V\_{genesis}$ Inheritance:** Child nodes monotonically inherit or geometrically combine parent $V\_{genesis}$ snapshots. |
...
| **T-11: Merge-Dilution Laundering** | Merging a drifted node $N\_A$ (near $\\tau\_{genesis} \= 0.70$) with a fresh node $N\_B$ to dilute $N\_A$'s composite genesis floor. | **Section 3.2 / 3.3 Conservative Multi-Parent Floor Check:** $N\_{child}$ MUST independently satisfy $\\tau\_{genesis} \\ge 0.70$ against **ALL** constituent parent genesis anchors. |
...
* **Rationale:** A cryptographic hash function $H$ (such as SHA-256) outputs a fixed-length byte digest that lacks geometric orientation in the $d$-dimensional embedding hypersphere $\\mathbb{S}^{d-1}$. Executing a dot product between an embedding payload vector $R \\in \\mathbb{R}^d$ and a raw cryptographic hash array is a dimensionally invalid operation. Normalized Weighted Centroid Composition produces a unit-normalized vector $\\mathbf{v}\_{genesis,child} \\in \\mathbb{S}^{d-1}$ that preserves geometric compatibility for cosine similarity evaluations, while cryptographic hashes remain strictly reserved for $t\_0$ provenance commitments and frame header checksums.

### **DD-1.5 Mitigation of Threat T-11 (Merge-Dilution Laundering) and Collective Governance Rules**

* **Decision:** Mandating that merged enclaves ($N\_{child}$) satisfy the genesis floor constraint ($\\tau\_{genesis} \\ge 0.70$) independently against **ALL** constituent parent genesis anchors ($V\_{genesis,i}$), and enforcing the Infimum Rule ($\\text{Tier}(N\_{coll}) \= \\min\_i \\text{Tier}(N\_i)$) for collective enclaves.  
* **Rationale:** If a composite anchor vector $\\mathbf{v}\_{genesis,child}$ replaced parent anchors completely, a heavily drifted node $N\_A$ sitting near $\\tau\_{genesis} \= 0.70$ could merge with a fresh, pristine node $N\_B$ ($\\tau\_{genesis} \= 1.00$). The midpoint vector blend would artificially move $N\_A$'s reference floor closer to its drifted state, granting $N\_A$ an unauthorized buffer to drift further without triggering an alert (Threat T-11). Requiring independent verification against all constituent parent genesis anchors eliminates this laundering path. Furthermore, setting collective operational tiers to the minimum tier among member nodes prevents provisional or compromised nodes from acquiring Tier III permissions by joining high-tier collectives.
